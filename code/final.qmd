...
The data cleaning and wrangling process involved excluding rows with missing microhabitat composition data and renaming columns for clarity. The missing data was visualized to assess the presence of missing values, which showed no missing values for mound locations or seed counts. To explore the data, a pairs plot was created to visualize the relationships between variables. A histogram of seed counts was also plotted to examine the distribution. For the statistical analysis, generalized linear models (GLMs) were built to investigate the relationship between seed count and the predictors of treatment, species, and number of inflorescences. Several GLM models were constructed, including models with different combinations of predictors and using either Poisson or negative binomial family distributions.

Assumption checks were performed on the models by examining the residuals. Models that did not meet the assumptions were excluded from further analysis, and the models that met the assumptions were compared using the Akaike Information Criterion (AIC) to determine the best model (model 11). Model summaries, confidence intervals, and adjusted R-squared values were calculated for this model. The results were reported using tables, with estimates, standard errors, t-stats, and p-values rounded to two digits.

To assess the significance of the relationship, the null hypothesis (H0) stated that there is no relationship between treatment, species, seed count, and the number of inflorescences. The alternative hypothesis (HA) stated that there is a relationship between these variables. The significance level was set at 0.05.
